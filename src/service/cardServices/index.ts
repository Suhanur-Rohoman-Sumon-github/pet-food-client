...
export const getMyCard = async (userId:string) => {
  const { data } = await axios.get(`http://localhost:3000/api/v1/products/my-card/${userId}`);
  return data.data; 
};
...
export const addToCard = async (userID :string,productId: string) => {
  ...
  const { data } = await axios.post(`http://localhost:3000/api/v1/products/add-to-card/${userID}/${productId}`);
